...
- **Filter by location AND by category** — filters only ever *narrow which markers show*, never change the kind of view (the control bar is **sticky** so it stays in reach while scrolling). All/All plots every location×category **segment** at once (each its own labelled purple marker); pick a location to narrow to its categories (+ its competitors); pick a category too for a single segment. A brand is routinely in different squares across its categories — that's the point.
- **Competitor markers are category-accurate.** When a category is selected, competitors come from `competitorsBySegment["<loc>|<cat>"]` (studio competitors for a studio segment, office competitors for coworking) — NOT the location-level set. This is a correctness requirement: the same market has entirely different rivals per category. `Show / hide competitors` toggles them off for a clean brand-only plot.
- **Hover any marker** — a tooltip shows that segment's exact Visibility %, Citation %, and its square.
- **Trends over time** — a line chart (like RankPrompt's own dashboard) under the plot: **Metric** (Citation Share ↔ Visibility Score) × **Compare** (Competitors / By platform / By category). Competitors plots the brand vs its top cited-domain rivals; By platform splits Visibility across ChatGPT/Perplexity/AI Overviews (visibility-only — per-platform citation isn't tracked); By category splits the brand across its real categories. Respects the Location filter (Location=All shows the brand's line per market) and, in Competitors mode, the Category filter. Dashed threshold line marks the square cutoff, **hover shows a crosshair + per-series breakdown at that date**, and changing location/category shows a brief "loading" state on the chart so the reload is legible. **This is the home for movement over time — the matrix does not animate.**
...
- `competitorsByLocation{}` — per location (all categories), `[{id, name, visibility, citation}]` for the location-level view.
- `competitorsBySegment{}` — keyed `"<location>|<category>"`, the same shape — the **category-accurate** competitors shown when a category is selected. Compute each competitor's visibility (cell appearance rate from `businesses_categorized`, matched to its `website` domain) and citation (that domain's share) within the category.
...
- `trends{}` — per location, the weekly series powering the trends chart: `[{date, you:{vis, cit, visByPlatform:{chatgpt, perplexity, ai_overviews}, byCategory:{<cat>:{vis, cit}}}, competitors:{<domain>:{cit, vis}}}]`. Keep the competitor domains consistent across all weeks (0 when absent) so lines are continuous. `byCategory` keys must be the report's **real** `category` values.
  - **Competitor visibility must be matched by business NAME, not by the `businesses_categorized.website` field.** Older reports populate the entity name but leave `website` null (it was added later), so a website-only match silently returns 0 for every early week and the competitor lines flat-line before the field appears. Match the competitor's name token against `businesses_categorized[].name`; use `website`/citation domain only for the citation metric.
