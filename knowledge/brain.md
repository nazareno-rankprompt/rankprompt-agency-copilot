...
**AI visibility** (mention rate) is the share of AI answers where the brand appears at all — `brand_appears` ÷ results. **Citation share** is the brand's own-domain citations divided by total citations — whether the brand's *own pages* are the source AI pulls from. They are **two different numbers measuring two different things**; always state both, label each by its name, and never report one under the other's label (a citation-share figure announced as "visibility" is the single most common and most damaging mistake here). The gap between them is usually the whole story. Read them as a 2×2 of high/low on each:
