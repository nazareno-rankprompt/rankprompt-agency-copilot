...
**AI visibility** (mention rate) is the share of AI answers where the brand appears at all — `brand_appears` ÷ results. **Citation share** is the brand's own-domain share of cited sources — whether the brand's *own pages* are the source AI pulls from. **Read RankPrompt's Citation Share figure (the dashboard's Citation Rank panel); do not hand-compute it. If you ever must, the denominator is the citations within the report/segment (`/citations/by-domain`), never the brand's all-time pool of every source — and it must reconcile with RankPrompt's number (28 own ÷ 4,030 all-time = a fake 0.7%; the real share was 3.8%).** They are **two different numbers measuring two different things**; always state both, label each by its name, and never report one under the other's label (a citation-share figure announced as "visibility" is the single most common and most damaging mistake here). The gap between them is usually the whole story. Read them as a 2×2 of high/low on each:
